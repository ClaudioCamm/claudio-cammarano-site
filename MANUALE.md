...
Corretto a settembre 2026, in due passaggi. Il primo tentativo ancorava la ricerca a `name:` **seguito da** `type:`, e ha retto finché l'ordine dei campi è rimasto costante: quando `related` e `geo` hanno cominciato a comparire fra i due, le voci interessate non venivano più trovate e l'articolo non veniva aggiunto — di nuovo in silenzio. L'ancora giusta è la **posizione nel file**, non l'ordine dei campi: la definizione è `name:` a inizio riga con quattro spazi di rientro, mentre un riferimento dentro `related` è `{ name: ... }` inline con sei. E dal 26 settembre, se una voce non si localizza, lo script lo dice invece di tirare dritto. La regola per chi metterà mano a questi script: *il nome di un concetto non è una chiave univoca nel file, lo è solo la sua definizione.* Se un giorno si aggiungessero altri campi che citano concetti per nome, vale lo stesso avvertimento.
...
**Deposito e DOI.** In `dataset/` ci sono `README.md` (schema, metodo, provenienza, limiti dichiarati), `CITATION.cff` e `zenodo-metadata.md` con i campi già compilati. Il caricamento su Zenodo va fatto a mano dal proprio account. Depositato il 19 settembre 2026. I DOI sono due e non sono intercambiabili: **10.5281/zenodo.22843731** è quello concettuale — non cambia mai, risolve sempre all’ultima versione, ed è quello che va citato e messo nelle pagine; **10.5281/zenodo.22843732** identifica la versione 1.0.0 e solo quella. Stanno entrambi in `src/_data/dataset.js` (`doi` e `versionDoi`) e finiscono entrambi in `identifier` dentro `/concetti.json`. Per le versioni successive: alzare `version` e `released`, scrivere il changelog, usare «New version» sullo stesso record Zenodo e aggiornare `versionDoi` con il DOI nuovo — `doi` non si tocca.
...
Ogni pagina ha la sua immagine per i social, generata in automatico durante la build (`npm run build`, quindi anche su Netlify; con `npm start` non si generano):

- **Home italiana e pagine senza immagine propria**: claim e carta «Dove guarda questo sito» (Lambert su Bergamo). **Home inglese e pagine inglesi**: claim inglese e carta di Goode.
- **Concetti**: nome del concetto e carta con evidenziati i suoi paesi (campo `geo`); i paesi troppo piccoli hanno un cerchio. Senza geografia: la carta del sito.
- **Curated**: titolo, fonte e carta con l'unione dei paesi dei concetti collegati.
