...
<div class="info-box">
  <p>This page is a living document, not a dated note. The rest of the Lab records what was true on the day each note was written; this page is maintained, and supersedes any figure in an older note that contradicts it. Last updated 28 August 2026.</p>
</div>
...
`ARlex` is 4.44%. `ARsem` is 76.67%. The gap is approximately 72 percentage points. Mean pairwise Jaccard similarity across narrative fields is 0.340, and the composite index is 0.596.
...
The multi-model control is designed and pre-registered but not executed. Until it runs, the lexical–semantic gap is a property of one model, and the architectural reading of it is an inference rather than a result.
...
The paper is at v6, under editorial review before deposit. The route is an OSF deposit and pre-registration, then arXiv as a preprint under cs.AI or cs.CL, with *Synthese* and *Episteme* as journal targets at six to twelve months.

The invitation stands. The Coding Manual, the thirty-one-claim corpus with full annotations, the system prompt, the extraction scripts and the blind packet are deposited openly, and Phase 2 needs annotators. Researchers or graduate students with a background in epistemology, philosophy of language or computational linguistics willing to code a sub-corpus of twenty to thirty claims are invited to write.
...
Documents: {% labdoc "cammarano-post-cognitive-validation-v6.pdf", "Working paper — v6" %}, {% labdoc "cammarano-post-cognitive-validation-backmatter-v6.pdf", "References & appendices — v6" %}, {% labdoc "cammarano-coding-manual-v2-1.pdf", "Coding Manual — v2.1" %} — August 2026.
